// thank to  @tskiofc & @kyuucode
// func poll,payment,status group,album,product from dugong.js
// thank claude ai for create code
...
export const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            if (typeof content === 'object' && 'album' in content && Array.isArray(content.album)) {
                const { randomBytes } = await import('crypto');
                const array = content.album;
                ...
                }, { userJid: authState.creds.me.id, quoted: options.quoted, upload: waUploadToServer });
                await relayMessage(jid, album.message, { messageId: album.key.id });
                for (const item of array) {
                    const img = await generateWAMessage(jid, item, { upload: waUploadToServer });
            ...
            if (typeof content === 'object' && 'payment' in content) {
                const data = content.payment;
                let notes = {};
                if (data.sticker?.stickerMessage) {
                    notes = { stickerMessage: { ...data.sticker.stickerMessage, contextInfo: { stanzaId: options.quoted?.key?.id, participant: options.quoted?.key?.participant, quotedMessage: options.quoted?.message } } };
                } else if (data.note) {
                    notes = { extendedTextMessage: { text: data.note, contextInfo: { stanzaId: options.quoted?.key?.id, participant: options.quoted?.key?.participant, quotedMessage: options.quoted?.message } } };
                }
                const payMsg = {
            ...
            if (typeof content === 'object' && 'groupStatusMessage' in content) {
                ...
                if (storyData.message) {
                    ...
                    waMsgContent = await generateWAMessageContent(storyData, {
                        upload: async (readStream, opts) => {
                            return await waUploadToServer(readStream, { ...opts, newsletter: false });
                        }
                    });
                }
